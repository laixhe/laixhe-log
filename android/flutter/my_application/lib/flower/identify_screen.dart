...
import 'identify_company_detail_screen.dart';
import 'identify_item_screen.dart';
...
class _IdentifyScreenState extends State<IdentifyScreen> {
  ...
  List<IdentifyCompany> _companyList = [];
  ...
  void initState() {
    ...
    // 注册侦听事件 - 用于滑动列表ListView时的高度以便加载下一页数据
    _scrollController.addListener(() {
      // _scrollController.position.pixels 当前停留的位置距离
      // _scrollController.position.maxScrollExtent 当前列表的最大外围距离
      // 判断是否滑动到底部了
      if(_scrollController.position.pixels == _scrollController.position.maxScrollExtent) {
        // 触发上拉调用
        print('T-------- loading more');
      }
    });
    _companyList = IdentifyCompany.fromJson("""
          {
            "list": [
               {
      ...
      body: RefreshIndicator(
        child: ListView.builder(
          ...
          itemBuilder: (BuildContext context, int index){
            // 列表的每一行
            return InkWell(
              child: IdentifyItemScreen(_companyList[index]),
              onTap: (){

                // 跳转页面
                Navigator.push(context,
                    new MaterialPageRoute(
                        builder: (context) => IdentifyCompanyDetailScreen()
                    )
                );
