...
fn game_setup(
    ...
    lives.0 = 3;
    score.0 = 0;
    info!("[状态] 进入 Game → Ready（关卡初始化：{}行 × {}列砖块，生命=3）",
...
fn check_collision(
    ...
    if ball_transform.translation.x + BALL_RADIUS >= window_half_width {
        ...
        info!("[碰撞] 撞右墙 @ x={:.1}，vx 反向 → {:.0}", ball_transform.translation.x, ball_velocity.x);
    } else if ball_transform.translation.x - BALL_RADIUS <= -window_half_width {
        ball_velocity.x = ball_velocity.x.abs();
        info!("[碰撞] 撞左墙 @ x={:.1}，vx 反向 → {:.0}", ball_transform.translation.x, ball_velocity.x);
    }
    ...
    if ball_transform.translation.y + BALL_RADIUS >= window_half_height {
        ...
        info!("[碰撞] 撞上墙 @ y={:.1}，vy 反向 → {:.0}", ball_transform.translation.y, ball_velocity.y);
    ...
    let ball_bounding_circle =
        BoundingCircle::new(ball_transform.translation.xy(), BALL_RADIUS);
    ...
    for (entity, transform) in &collider_query {
        ...
        if ball_bounding_circle.intersects(&collision_entity_bounding_box) {
            ...
            if paddle_query.get(entity).is_ok() {
                ...
                info!("[碰撞] 击中拍子 @ ({:.1}, {:.1})，偏移={:.2}，反弹角={:.0}°，速度={:.0}",
                    ball_transform.translation.x, ball_transform.translation.y,
                    impact, angle.to_degrees(), speed);
            }
...
fn on_brick_collision(
    ...
    score.0 += 1;
    info!("[碰撞] 击中砖块 @ ({:.1}, {:.1})，nudge=({:.1},{:.1}) | 已击碎: {}",
        collision.ball_pos.x, collision.ball_pos.y,
        collision.nudge.x, collision.nudge.y, score.0);
}
...
fn on_collision(
    ...
    info!("[碰撞] 球加速：{:.0} → {:.0}", old_speed, ball_velocity.length());
}
