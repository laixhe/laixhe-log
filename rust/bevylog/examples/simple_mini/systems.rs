...
pub fn setup(mut commands: Commands) {
    ...
    // 这里用 commands.spawn((...)) 元组而非 bsn!：Sprite::from_color 是组件 Sprite 上的
    // 关联函数、返回 Sprite；而 bsn! 操作的是 Sprite 的模板类型 SpriteTemplate（由
    // FromTemplate 派生，其 image 字段被替换成 HandleTemplate<Image>），无法直接调用
    // from_color 这种返回 Sprite 的构造器。纯色精灵用元组 + from_color 更直观。
    commands.spawn((
...
pub fn spawn_bullet(mut reader: MessageReader<BulletFired>, mut commands: Commands) {
...
pub fn cleanup_bullets(bullets: Query<(Entity, &Transform), With<Bullet>>, mut commands: Commands) {
