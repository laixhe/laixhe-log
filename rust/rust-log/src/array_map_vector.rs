...
pub fn tuple_vec_map_run() {
    ...
    // 获取某个值(没有这个 索引 时会报错)
    println!("array_data get = {}", array_data[2]);

    // 判断是否包含某个值
    if array_data.contains(&9) {
    }

    // 判断 索引(下标) 是否存在
    let is_i: bool = match array_data.get(3) {
        Some(_i) => true,
        None => false,
    };
    println!("array_data is 3 = {}", is_i); // 结果: true
    ...
    println!("vector={:?}, len={}", vectors, vectors.len()); // 结果： [1, 9, 4]
